...
static void pickLetters() {
	...
	for(CubeID cid : CubeSet::connected()) {
		if(i == 0) cubeLetters[cid] = 'a' + words[index][0];
		else if(i == 1) cubeLetters[cid] = 'a' + words[index][1];
		else if(i == 2) cubeLetters[cid] = 'a' + words[index][2];
		else cubeLetters[cid] = 'a' + random.randrange(26);
		vbuf[cid].bg0.image(vec(0,0), Alphabet, cubeLetters[cid] - 'a');
		i++;
    }
}
